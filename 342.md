...
Bahut accha sawaal 🔥 — ye wali solution **most standard / optimized** hai LeetCode 342 (*Power of Four*) ke liye.
Chalo line by line breakdown karte hain 👇
...
* Binary form:
  ...
  ```
* Matlab isme **sirf odd-numbered bits (0, 2, 4, … positions)** set hain.

👉 Kyun use karte hain?

* Power of 2 hone ka matlab hai ek hi `1` bit set hai.
* Power of 4 hone ke liye, wo `1` hamesha **even index (0, 2, 4, …)** par hona chahiye.
...
---
# gyaan ki baat 
Bilkul — achhe se tod ke samjhata hoon. 🧠👇
Hum specifically line `return (n & 0x55555555) != 0;` kis tarah kaam karti hai, woh samjhenge.

# 1) Context — kyun pehle `power of two` check kar chuke hain
...
Simpler: mask ka bit pattern (right → left) = ... 0 1 0 1 0 1 0 1
Iska matlab: bit0 = 1, bit1 = 0, bit2 = 1, bit3 = 0, ...

(agar 64-bit long chahiye toh equivalent mask `0x5555555555555555L` hota hai)

# 3) `(n & mask) != 0` ka logic — kya check ho raha hai?

* Hum jaante hain `n` ek power of two — matlab **sirf ek bit set** hai, kahin pe.
* `n & mask` check karta hai: kya woh *single set bit* mask ke `1` bits ke saath overlap karta hai?

  * Agar overlap hua (result non-zero) → set bit mask ke allowed positions (even positions) par hai → **power of 4**.
  * Agar overlap zero → set bit mask ke `0` positions (odd positions) par hai → **power of 2 but not of 4** (jaise 2, 8, 32).

# 4) Examples (short binary dry runs)

* `n = 1` → `...0001`
  mask & n = `...0001` → non-zero → **true** (1 = 4^0)

* `n = 2` → `...0010`
  mask & n = `...0000` → zero → **false** (2 is 2^1, not 4^k)

* `n = 4` → `...0100`
  mask & n = `...0100` → non-zero → **true** (4 = 4^1)

* `n = 8` → `...1000`
  mask & n = `...0000` → zero → **false**

* `n = 16` → `...10000`
  mask & n = `...10000` → non-zero → **true**
...
AND →                0 0 0 0 0 1 0 0 → non-zero → OK

# 6) Kyon yeh safe / correct approach hai

* Pehle `n & (n-1) == 0` se ensure hua ke **ek hi bit set** hai.
* Fir mask se ensure hota hai ki wo **set bit even index (0,2,4,...)** mein hai — wahi condition for being 4^k.
...
Agar chaho to main ab isko ek short code-snippet ke saath 3–4 example calls karke dikhadein (dry run) ya phir arithmetic alternative bhi bata doon (jaise repeatedly divide by 4). Kaunsa chahoge?


Yes 🚀 tumne ekdum sahi socha.
...
* `0x55555555` → even bit mask (positions 0,2,4,6…)
* `0xAAAAAAAA` → odd bit mask (positions 1,3,5,7…)

---

Bataun kya tumhe ek **table bana ke** dikhau jisme numbers (1–32) ka binary aur ye mask apply karke kaun pass hota hai kaun fail, taaki tumhe pura visual clear ho jaye?
